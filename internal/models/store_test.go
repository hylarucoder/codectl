...
func withEnv(t *testing.T, key, val string) func() {
    ...
    return func() {
        if had { _ = os.Setenv(key, old) } else { _ = os.Unsetenv(key) }
    }
}
...
func TestModels_SaveLoad_AddRemove(t *testing.T) {
    ...
    if len(got) != 0 { t.Fatalf("expected empty models, got %v", got) }
    ...
    if !hasAll(added, []string{"m1", "m2"}) { t.Fatalf("unexpected added: %v", added) }
    if !hasAll(existed, []string{"m1"}) { t.Fatalf("unexpected existed: %v", existed) }
