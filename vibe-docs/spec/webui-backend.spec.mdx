...
- 嵌入前端：`go:embed` 打包 `web/dist`，以 `/*` 根路径提供，支持 SPA 回退。
...
- `GET /*path` → 从 `embed.DistFS` 提供静态资源，未知路径回退 `index.html`（SPA）。
...
func Router(cfg *config.Config, gdb *gorm.DB) *gin.Engine {
  ...
  mountEmbeddedUI(r) // serve /* with index fallback
  return r
}
```
...
- `go build`：若 `web/dist` 存在则嵌入；否则访问前端路径返回 404 并输出提示。
- `codectl serve --open`：启动后自动打开浏览器（`http://127.0.0.1:8080/`）。

## 验收标准

- `codectl serve` 能启动 Gin 服务器，默认绑定 `127.0.0.1:8080`。
- 默认数据库为 sqlite：未设置 `DB_DRIVER` 时在 `~/.codectl/codectl.db` 创建并运行迁移。
- 访问 `/` 能加载嵌入的前端（若内置 `web/dist` 存在）。
