...
- `codectl model ls-remote`：列出远端可用的模型清单（默认从 `~/.codectl/provider.yaml` 读取；若缺失则使用内置清单）。
...
- `codectl mcp ls-remote`：列出远端可用的 MCP 服务端清单（默认从 `~/.codectl/provider.yaml` 读取；若缺失则使用内置清单）。
