...
- 核心能力：
  ...
  - CLI 子命令：`add`、`remove`、`update`、`upgrade`、`version`、`config`、`spec`。
...
### 6.3 CLI 子命令
- `codectl`：启动 TUI。
- `codectl add [tool]`：安装工具（默认 all；支持 `codex|claude|gemini`）。
- `codectl remove [tool]`：卸载工具（默认 all）。
- `codectl update [tool]`：升级工具（默认 all）。
- `codectl upgrade`：自更新（占位）。
...
### 6.5 升级策略
- 触发：TUI `u` 或 `/upgrade`；CLI `update`/`add`。
