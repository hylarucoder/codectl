...
- `codectl model ls|ls-remote|add|remove`：管理本地模型与查看远端清单（远端来自 `provider.json(v2)` 扁平化）。
- `codectl mcp ls|ls-remote|add|remove`：管理本地 MCP 服务端清单与占位远端清单。
...
- 0.1 原型：TUI 框架、Spec UI 雏形与基础交互。
- 0.2 健康检查：检测 CLI 是否已安装与版本信息；API Key/代理连通性与修复建议（逐步完善）。
- 0.3 配置向导：
  - 引导写入/更新环境变量与常见 CLI 的配置文件。
  - 支持创建与切换配置档（profiles）。
- 0.4 生态与 MCP：
  - 统一配置（`~/.codectl/*.json`），MCP 客户端/服务端的基础管理能力（配置与健康检查）。
  - 常见编辑器/终端插件的配置提示与自动化脚本。
- 0.5 可扩展提供商：更多 LLM/企业代理/私有化部署；Provider/MCP 插件机制（提案与 PoC）。
