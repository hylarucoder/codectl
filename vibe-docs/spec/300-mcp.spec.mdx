...
- `codectl mcp list [--json]`
  - MUST 列出合并后有效的 MCP 服务器，含 `id|name|enabled|status|transport|endpoint|autostart`。
  - `--json` 输出 MUST 符合 `ListOutput.schema`（后续补充）。
  - 退出码：0 成功；>0 读取/解析错误。

- `codectl mcp add <manifest>`
  - 从本地文件（json/yaml）或远端 URL 导入一个服务器定义。
  - MUST 为导入的服务器生成唯一 `id`（或使用清单内提供的 id）。
  - 冲突（同 id）：MUST 询问覆盖/新 id（非交互模式下默认失败并退出码=2）。

- `codectl mcp remove <id>`：从当前作用域配置中移除（不影响其他层）。
...
- `codectl mcp export <id> [--format=json|yaml]`
  - 导出单个服务器的独立 manifest（便于分享/复用）。
...
- `codectl mcp add`/`enable` 等操作 MUST 不回显密钥内容。
