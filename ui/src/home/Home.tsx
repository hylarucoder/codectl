...
const Home: React.FC = () => {
  ...
  const [agent] = useXAgent<BubbleDataType>({
    ...
    // NOTE: do not commit real secrets; replace at runtime
  ...
  const { onRequest, messages, setMessages } = useXChat({
    ...
    transformMessage: (info) => {
      const { originMessage, chunk } = (info || {}) as any
      let currentContent = ''
      let currentThink = ''
      try {
      ...
      <Conversations
        ...
        onActiveChange={async (val) => {
          ...
          setTimeout(() => {
            ...
            setMessages((messageHistory as any)?.[val] || [])
          }, 100)
        }}
        groupable
              ...
              onClick: () => {
                ...
                setTimeout(() => {
                  if (conversation.key === curConversation) {
                    setCurConversation(newKey as string)
                    setMessages((messageHistory as any)?.[newKey as string] || [])
                  }
                }, 200)
              },
            },
        ...
        >
          ...
          <Flex gap={16}>
            <Prompts
              ...
              onItemClick={(info) => {
                onSubmit((info as any).data.description as string)
              }}
